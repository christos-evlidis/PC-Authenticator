import { dataGetCounter } from "./counter.js";
import { dataSanitizeSecret } from "../records/sanitize.js";

/** Generates a TOTP or HOTP code for the given secret and options. */
function dataGetCode(secret, options) {
  try {
    const sanitized = dataSanitizeSecret(secret);
    const counterValue = dataGetCounter(options);
    const hmacFn = {
    ...
    if (!hmacFn) {
      console.warn(
        `[data-otp] dataGetCode: unsupported algorithm ${options.algorithm}`,
      );
    ...
    console.warn("[data-otp] dataGetCode failed", error);
