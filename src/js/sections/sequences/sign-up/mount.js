...
/** Inserts the sign-up intro overlay shell when the load intro has already finished. */
function signUpAnimationMount() {
  ...
  intro.className = `app-intro ${INTRO_SIGN_UP_STAGED_CLASS}`;
...
/** Removes the sign-up intro overlay shell. */
function signUpAnimationUnmount() {
  document.querySelector(INTRO_ROOT_SELECTOR)?.remove();
}

export { signUpAnimationMount };
export { signUpAnimationUnmount };
