...
export async function bodyAnimateForSignedInContent(options = {}) {
  ...
  if (reset) {
    ...
    display.textContent = "";
    display.classList.remove(BODY_RUNNING_CLASS, BODY_SIGNED_IN_TEXT_TYPE_CLASS);
    display.removeAttribute("data-full-text");

    icon?.classList.remove(BODY_RUNNING_CLASS, BODY_ACTIVE_CLASS, BODY_SIGNED_IN_ICON_POP_CLASS);
  ...
  if (isStatic) {
    ...
    icon?.classList.remove(BODY_PENDING_CLASS, BODY_RUNNING_CLASS, BODY_SIGNED_IN_ICON_POP_CLASS);
    icon?.classList.add(BODY_ACTIVE_CLASS);
    return;
  }

  const iconPopMs = cssMs(root, BODY_VAR_SIGNED_IN_ICON_POP_MS);
  const textTypeMs = cssMs(root, BODY_VAR_SIGNED_IN_TEXT_TYPE_MS);

  if (icon) {
    icon.classList.remove(BODY_PENDING_CLASS);
    icon.classList.add(BODY_SIGNED_IN_ICON_POP_CLASS);
    await waitForAnimationEnd(
      icon,
      "bodySignedInIconPop",
      iconPopMs + BODY_ANIMATION_TIMEOUT_BUFFER_MS,
    );
    icon.classList.remove(BODY_SIGNED_IN_ICON_POP_CLASS);
    icon.classList.add(BODY_ACTIVE_CLASS);
  }

  if (stack && display) {
    display.textContent = "";
    display.classList.add(BODY_SIGNED_IN_TEXT_TYPE_CLASS, BODY_RUNNING_CLASS);
    ...
    display.classList.remove(BODY_SIGNED_IN_TEXT_TYPE_CLASS, BODY_RUNNING_CLASS);
