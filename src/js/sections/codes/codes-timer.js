...
function hasTotpCards() {
  return getCardRoots().some((root) => accountOtpApi().accountTotpIs(root.account));
}

function getFirstTotpRoot() {
  return (
    getCardRoots().find((root) => accountOtpApi().accountTotpIs(root.account)) ?? null
  );
}

export function updateAccountCode(root) {
  const { account, els, card } = root;
  const otpOptions = accountOtpApi().accountOtpOptionsGet(account);
  const otp = accountOtpApi().accountOtpNumberGet(account.secret, otpOptions);
...
function runSecondTick() {
  ...
  for (const root of getCardRoots()) {
    if (accountOtpApi().accountHotpIs(root.account)) {
      continue;
    }

    const clock = accountOtpApi().accountOtpClockGet(
      accountOtpApi().accountOtpOptionsGet(root.account),
    );
...
export function startTicker() {
  ...
  handlePeriodRollover(
    accountOtpApi().accountOtpClockGet(
      firstTotp ? accountOtpApi().accountOtpOptionsGet(firstTotp.account) : {},
    ),
  );
...
export function getHotpCounterValue(account) {
  return Number.isInteger(account?.counter)
    ? account.counter
    : accountOtpApi().HOTP_DEFAULT_COUNTER;
}
...
export function parseHotpCounterInput(text) {
  ...
  if (!Number.isInteger(counter) || counter < accountOtpApi().MIN_COUNTER) {
    return null;
  }
